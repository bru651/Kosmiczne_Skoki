...
MovingCircle::MovingCircle(float radius, float angle, float speed, float size, const std::string& textureFile, float mass, int id, bool walkable)
    ...
    sprite.setOrigin(256, 256); // Center the sprite (512/2)
    sprite.setScale((2 * size) / 512.0f, (2 * size) / 512.0f); // Scale to match circle size
}
...
void MovingCircle::update(float deltaTime, const sf::Vector2f& center) {
    ...
    shape.setPosition(position); // For debugging or placeholder
    sprite.setPosition(position); // Update sprite position

    for (auto& building : buildings) {
        building.updatePosition(position, size);  // update all buildings
    }
}
